...
function spam(){
    ...
    let form =document.forms["spam"]
    form.elements["vk"].forEach(day => {
        day.checked && req.vk.days.push(day.value)
    });
    req.vk.time=form.elements["cronvk"].value
    form.elements["tg"].forEach(day => {
        day.checked && req.tg.days.push(day.value)
    });
    req.tg.time=form.elements["crontg"].value
    console.log(req)
    var promis = axios.post("http://httpbin.org/post",req);
}
